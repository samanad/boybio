...
class Uploads {
    ...
    public static function process_upload($already_existing_file_name, $uploads_file_key, $file_key, $file_key_remove, $size_limit, $error_response_type = 'error', $error_field = null, $force_local = false) {
        /* Determine the error response */
        $return_error = null;
        switch($error_response_type) {
            case 'error':
        ...
        if(!empty($_FILES[$file_key]['name'])) {
            ...
            if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                ...
                /* Offload uploading */
                if(\Altum\Plugin::is_active('offload') && settings()->offload->uploads_url && !$force_local) {
                    try {
                        $s3 = new \Aws\S3\S3Client(get_aws_s3_config());
