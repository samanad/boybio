...
function get_aws_s3_config() {
    ...
    error_reporting(0); // Completely suppress errors during AWS config
    
    /* Set environment variable to prevent AWS SDK from trying to read config file */
    if(!isset($_ENV['AWS_CONFIG_FILE'])) {
        $_ENV['AWS_CONFIG_FILE'] = '';
        putenv('AWS_CONFIG_FILE=');
    }
    if(!isset($_ENV['AWS_SHARED_CREDENTIALS_FILE'])) {
        $_ENV['AWS_SHARED_CREDENTIALS_FILE'] = '';
