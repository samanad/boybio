...
return [
    ...
    'admin_settings.users.blacklisted_countries' => 'Selected countries',
    'admin_settings.users.blacklisted_countries_help' => 'Countries used by the mode above. In “Only access” mode these are the only countries that can use the site. In “Block” mode these countries are denied. Leave empty (or set mode to Disabled) to turn off restrictions.',
    ...
    'admin_settings.security.biolink_edit_allowed_ip' => 'Admin Auto-Login IPs',
    'admin_settings.security.biolink_edit_allowed_ip_help' => 'One or more IP addresses that auto-login as the first active admin when accessing user or admin pages. Enter one IP per line (commas also work). Leave empty to disable.',
    'admin_settings.security.biolink_edit_allowed_ip_error' => 'Please provide valid IP addresses only (one per line).',
    'admin_settings.security.google_login_persistent_ip' => 'Google Login Persistent IPs',
    'admin_settings.security.google_login_persistent_ip_help' => 'One or more IPs where Google login enables persistent login (long cookies + device cookie that survives logout). While on any of these IPs, matching users are auto-logged back in. Enter one IP per line. Leave empty to disable.',
    'admin_settings.security.google_login_persistent_ip_error' => 'Please provide valid IP addresses only (one per line).',
    ...
    'admin_settings.links.directory_display' => 'Directory biolinks to display (Guests only)',
    ...
    'admin_settings.links.directory_guest_links_placeholder' => 'gave' . "\n" . 'sad' . "\n" . 'example',
    ...
    'admin_settings.links.directory_access' => 'Directory modes (3 audiences)',
    'admin_settings.links.directory_access_help' => 'The directory at /directory shows different biolinks depending on who is viewing. All modes still require the link to be a biolink, enabled, and to have "directory displaying" turned on for that link.<br><br><strong>1. Admin</strong> — sees only biolinks with a verified badge (users request verification; you approve it in Admin → Links).<br><br><strong>2. Logged-in users (Explore Things)</strong> — sees only biolinks marked Explore Things. Toggle this per link in Admin → Links → Add/Remove Explore Things (users cannot set this themselves).<br><br><strong>3. Guests (not logged in)</strong> — sees only the URL slugs you list below under "Directory biolinks to display", grouped per domain. Leave a domain empty to show nothing to guests for that domain.',
