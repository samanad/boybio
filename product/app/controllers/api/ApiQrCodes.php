    ...
    private function post() {
        ...
        switch ($_POST['type']) {
            ...
            case 'upi':
                ...
                $settings['upi_amount'] = $_POST['upi_amount'] = (float)($_POST['upi_amount'] ?? null);
            ...
            case 'epc':
                ...
                $settings['epc_iban'] = $_POST['epc_iban'] = input_clean($_POST['epc_iban'] ?? null, $available_qr_codes['epc']['iban']['max_length']);
                $settings['epc_payee_name'] = $_POST['epc_payee_name'] = input_clean($_POST['epc_payee_name'] ?? null, $available_qr_codes['epc']['payee_name']['max_length']);
                $settings['epc_currency'] = $_POST['epc_currency'] = in_array($_POST['epc_currency'], ['EUR']) ? $_POST['epc_currency'] : 'EUR';
                $settings['epc_amount'] = $_POST['epc_amount'] = (float)($_POST['epc_amount'] ?? null);
                $settings['epc_bic'] = $_POST['epc_bic'] = input_clean($_POST['epc_bic'] ?? null, $available_qr_codes['epc']['bic']['max_length']);
                $settings['epc_remittance_reference'] = $_POST['epc_remittance_reference'] = input_clean($_POST['epc_remittance_reference'] ?? null, $available_qr_codes['epc']['remittance_reference']['max_length']);
                $settings['epc_remittance_text'] = $_POST['epc_remittance_text'] = input_clean($_POST['epc_remittance_text'] ?? null, $available_qr_codes['epc']['remittance_text']['max_length']);
                $settings['information'] = $_POST['information'] = input_clean($_POST['information'] ?? null, $available_qr_codes['epc']['information']['max_length']);
                break;
            ...
            case 'pix':
                ...
                $settings['pix_payee_key'] = $_POST['pix_payee_key'] = input_clean($_POST['pix_payee_key'] ?? null, $available_qr_codes['pix']['payee_key']['max_length']);
                $settings['pix_payee_name'] = $_POST['pix_payee_name'] = input_clean($_POST['pix_payee_name'] ?? null, $available_qr_codes['pix']['payee_name']['max_length']);
                $settings['pix_currency'] = $_POST['pix_currency'] = in_array($_POST['pix_currency'], ['BRL']) ? $_POST['pix_currency'] : 'BRL';
                $settings['pix_amount'] = $_POST['pix_amount'] = (float)($_POST['pix_amount'] ?? null);
                $settings['pix_city'] = $_POST['pix_city'] = input_clean($_POST['pix_city'] ?? null, $available_qr_codes['pix']['city']['max_length']);
                $settings['pix_transaction_id'] = $_POST['pix_transaction_id'] = input_clean($_POST['pix_transaction_id'] ?? null, $available_qr_codes['pix']['transaction_id']['max_length']);
                $settings['pix_description'] = $_POST['pix_description'] = input_clean($_POST['pix_description'] ?? null, $available_qr_codes['pix']['description']['max_length']);
                break;
        }
                    ...
                    if(\Altum\Plugin::is_active('offload') && settings()->offload->uploads_url) {
                        try {
                            ...
                            $result = $s3->putObject([
                                ...
                                'Key' => UPLOADS_URL_PATH . Uploads::get_path('qr_code_logo') . $image_new_name,
                        ...
                        /* Upload the original */
                        move_uploaded_file($file_temp, Uploads::get_full_path('qr_code_logo') . $image_new_name);
                    }
    ...
    private function patch() {
        ...
        switch ($_POST['type']) {
            ...
            case 'pix':
                ...
                $settings['pix_amount'] = $_POST['pix_amount'] = (float)($_POST['pix_amount'] ?? $qr_code->settings->pix_amount);
            ...
            if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                ...
                if(\Altum\Plugin::is_active('offload') && settings()->offload->uploads_url) {
                    try {
                        ...
                        $s3->deleteObject([
                            'Bucket' => settings()->offload->storage_name,
                            'Key' => UPLOADS_URL_PATH . Uploads::get_path('qr_code_logo') . $qr_code->qr_code_logo,
                        ]);
                ...
                /* Local uploading */
                    ...
                    /* Delete current image */
                    if(!empty($qr_code->qr_code_logo) && file_exists(Uploads::get_full_path('qr_code_logo') . $qr_code->qr_code_logo)) {
                        unlink(Uploads::get_full_path('qr_code_logo') . $qr_code->qr_code_logo);
                    }
