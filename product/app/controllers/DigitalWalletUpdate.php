...
class DigitalWalletUpdate extends Controller {
    ...
    public function index() {
        ...
        /* Team checks */
        if(\Altum\Teams::is_delegated() && !\Altum\Teams::has_access('update.digital_wallets')) {
        ...
        if(!$digital_wallet = db()->where('digital_wallet_id', $digital_wallet_id)->where('user_id', $this->user->user_id)->getOne('digital_wallets')) {
            redirect('digital-wallets');
        }
        ...
        if(!empty($_POST)) {
            ...
            if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                ...
                redirect('digital-wallet-update/' . $digital_wallet_id);
            }
        }

        $values = [
            'name' => $_POST['name'] ?? $digital_wallet->name,
            'title' => $_POST['title'] ?? ($digital_wallet->settings->title ?? ''),
            'subtitle' => $_POST['subtitle'] ?? ($digital_wallet->settings->subtitle ?? ''),
            'link_id' => $_POST['link_id'] ?? $digital_wallet->link_id,
            'location_url' => $_POST['location_url'] ?? $digital_wallet->location_url,
            'background_color' => $_POST['background_color'] ?? ($digital_wallet->settings->background_color ?? '#111827'),
            'phone' => $_POST['phone'] ?? ($digital_wallet->settings->phone ?? ''),
            'email' => $_POST['email'] ?? ($digital_wallet->settings->email ?? ''),
            'website' => $_POST['website'] ?? ($digital_wallet->settings->website ?? ''),
        ];
        ...
        $view = new \Altum\View('digital-wallet-update/index', (array) $this);
