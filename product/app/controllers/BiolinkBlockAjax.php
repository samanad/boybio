...
class BiolinkBlockAjax extends Controller {
    ...
    /**
     * Safely get the order value for new biolink blocks
     */
    private function get_new_block_order() {
        $position = isset(settings()->links->biolinks_new_blocks_position) && settings()->links->biolinks_new_blocks_position == 'top' ? 'top' : 'bottom';
        return $position == 'top' ? -$this->total_biolink_blocks : $this->total_biolink_blocks;
    }

    public function index() {
        \Altum\Authentication::guard();

        /* Start output buffering to catch any warnings/notices - use multiple levels for safety */
        while(ob_get_level()) {
            ob_end_clean();
        }
        ob_start();
        
        /* Suppress ALL errors during execution to prevent JSON corruption */
        ...
        try {
            ...
            /* Restore error reporting and return error */
            error_reporting($original_error_reporting);
            Response::json(l('global.error_message.basic'), 'error');
        } catch (\Error $e) {
            /* Clean any output before sending JSON */
        ...
        /* Clean output buffer before die() - this should not be reached as Response::json() calls die() */
        while(ob_get_level()) {
            ob_end_clean();
        }
        die();
    ...
    private function update_biolink_vcard() {
        ...
        if(isset($_POST['vcard_avatar_remove'])) {
            ...
            if(\Altum\Plugin::is_active('offload') && settings()->offload->uploads_url) {
                /* Suppress AWS SDK warnings about config file outside open_basedir */
                $original_error_reporting = error_reporting();
    ...
    private function create_biolink_image() {
        $_POST['link_id'] = (int) $_POST['link_id'];
        $_POST['location_url'] = !empty($_POST['location_url']) ? get_url($_POST['location_url']) : '';
        ...
        /* Image upload - required for image block */
    ...
    public function handle_file_upload($already_existing_file, $file_name, $file_name_remove, $allowed_extensions, $upload_folder, $size_limit) {
        ...
        if($file) {
            ...
            if(\Altum\Plugin::is_active('image-optimizer') && isset(settings()->image_optimizer) && is_object(settings()->image_optimizer) && isset(settings()->image_optimizer->is_enabled) && settings()->image_optimizer->is_enabled) {
