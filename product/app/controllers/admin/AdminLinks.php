...
class AdminLinks extends Controller {
    ...
    public function index() {
        ...
        $filters = (new \Altum\Filters(['is_enabled', 'link_id', 'user_id', 'project_id', 'domain_id', 'type', 'is_verified', 'is_banned', 'is_explore_things', 'biolink_theme_id'], ['url', 'location_url'], ['link_id', 'last_datetime', 'datetime', 'url', 'location_url', 'clicks']));
    ...
    public function is_banned() {
        ...
        if(!$link = db()->where('link_id', $link_id)->getOne('links', ['link_id', 'type', 'is_banned'])) {
            redirect('admin/links');
        }
        ...
        if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
            ...
            db()->where('link_id', $link->link_id)->update('links', [
                'is_banned' => (int) !($link->is_banned ?? 0),
            ]);
