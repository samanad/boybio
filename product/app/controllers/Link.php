        ...
        switch($method) {
            case 'settings':
                ...
                if($this->link->type == 'biolink') {
                    ...
                    $link_links_result = database()->query("SELECT * FROM `biolinks_blocks` WHERE `link_id` = {$this->link->link_id} " . biolinks_blocks_order_by_sql());

                    /* Add the modals for creating the links inside the biolink */
                    foreach($biolink_blocks as $key => $value) {
                        if(!isset(settings()->links->available_biolink_blocks->{$key}) || !settings()->links->available_biolink_blocks->{$key}) continue;
