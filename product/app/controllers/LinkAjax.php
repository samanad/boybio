...
class LinkAjax extends Controller {
    ...
    public function index() {
        ...
        if(!empty($_POST) && isset($_POST['request_type'])) {
            /* Check CSRF validation - allow if strict validation is disabled */
            ...
            /* If CSRF check failed and strict validation is enabled, return error */
            if(!$csrf_check_passed && $csrf_strict_validation) {
                Response::json(l('global.error_message.invalid_csrf_token'), 'error');
            }
            
            /* Proceed if CSRF passed OR strict validation is disabled */
            /* When strict validation is disabled, allow requests even with missing/invalid tokens */
            if($csrf_check_passed || !$csrf_strict_validation) {
                ...
                /* If we reach here, something went wrong - return proper error */
                Response::json(l('global.error_message.basic'), 'error');
            }
        } else {
            /* No POST data or missing request_type */
            Response::json(l('global.error_message.basic'), 'error');
        }
        ...
        if($file_extension == 'zip') {
            ...
            if($zip->open($file_temp) === true) {
                ...
                for($i = 0; $i < $zip->numFiles; $i++) {
                    $OnlyFileName = $zip->getNameIndex($i);
                    $FullFileName = $zip->statIndex($i);
                    $file_extension = mb_strtolower(pathinfo($OnlyFileName, PATHINFO_EXTENSION));

                    if(!($FullFileName['name'][strlen($FullFileName['name'])-1] == "/") && !str_contains($FullFileName['name'], '__MACOSX')) {
                        if(in_array($file_extension, \Altum\Uploads::$uploads['static']['inside_zip_whitelisted_file_extensions'])) {
    ...
    private function update_link() {
        ...
        /* Prepare update data - check if email_reports_count column exists */
        $update_data = [
        ...
        /* Check if email_reports_count column exists before trying to update it */
        try {
            $column_check = database()->query("SHOW COLUMNS FROM `links` LIKE 'email_reports_count'");
            if($column_check->num_rows > 0) {
                $update_data['email_reports_count'] = count($_POST['email_reports']);
            }
        } catch(\Exception $e) {
            /* Column doesn't exist, skip it */
        }

        db()->where('link_id', $_POST['link_id'])->update('links', $update_data);
    ...
    private function update_biolink() {
        ...
        /* Exclusivity check */
        $_POST['is_main_link'] = isset($_POST['is_main_link']) && $domain_id && isset($domains[$_POST['domain_id']]) && $domains[$_POST['domain_id']]->type == 0;
        ...
        if($tool_tag_link_ids) {
            $allowed_tag_ids = [];
            $allowed_tag_result = database()->query("SELECT `link_id` FROM `links` WHERE `user_id` = {$this->user->user_id} AND `type` = 'biolink' AND `link_id` != {$link->link_id}");
            while($allowed_tag_row = $allowed_tag_result->fetch_object()) {
                $allowed_tag_ids[] = (int) $allowed_tag_row->link_id;
            }
            $tool_tag_link_ids = array_values(array_filter($tool_tag_link_ids, function($tag_link_id) use ($allowed_tag_ids) {
                return in_array($tag_link_id, $allowed_tag_ids, true);
            }));
            $tool_tag_link_ids = array_slice($tool_tag_link_ids, 0, 30);
        }
        $tool_tag_settings = [
        ...
        $_POST['pwa_display_install_bar_delay'] = max(1, (int) (isset($_POST['pwa_display_install_bar_delay']) ? $_POST['pwa_display_install_bar_delay'] : 3));
        ...
        /* Update the record */
        db()->where('link_id', $link->link_id)->update('links', $update_data);
