...
<head>
    ...
    <?php foreach(['custom.css', 'libraries/select2.css'] as $file): ?>
        <link href="<?= ASSETS_FULL_URL . 'css/' . $file . '?v=' . PRODUCT_CODE . (is_file(ASSETS_PATH . 'css/' . $file) ? '.' . filemtime(ASSETS_PATH . 'css/' . $file) : '') ?>" rel="stylesheet" media="screen,print">
    <?php endforeach ?>
...
<body class="<?= l('direction') == 'rtl' ? 'rtl' : null ?> app <?= \Altum\ThemeStyle::get() == 'dark' ? 'cc--darkmode' : null ?>" data-theme-style="<?= \Altum\ThemeStyle::get() ?>">
    ...
    (function() {
        ...
        // ONLY redirect on initial launch, not during normal navigation
        if (isPWAStandalone && isInitialLaunch) {
            const storedStartUrl = localStorage.getItem(PWA_START_URL_KEY);
            
            // For locked users, always use /fa/chats as the start URL
            ...
            if (isUserLocked && !storedStartUrl) {
