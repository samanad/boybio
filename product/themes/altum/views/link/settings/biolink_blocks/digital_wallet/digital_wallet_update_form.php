...
<form id="<?= 'update_biolink_block_' . $row->biolink_block_id ?>" name="update_biolink_" method="post" role="form" data-type="<?= $row->type ?>">
    ...
    <div class="form-group">
        <div class="d-flex flex-wrap flex-row justify-content-between">
            <label for="<?= 'digital_wallet_digital_wallet_id_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-wifi fa-sm text-muted mr-1"></i> <?= l('biolink_digital_wallet.digital_wallet_id') ?></label>
        ...
        <select id="<?= 'digital_wallet_digital_wallet_id_' . $row->biolink_block_id ?>" name="digital_wallet_id" class="custom-select" required="required">
			<?php foreach($digital_wallets as $digital_wallet): ?>
                <option value="<?= $digital_wallet->digital_wallet_id ?>" <?= ($row->settings->digital_wallet_id ?? null) == $digital_wallet->digital_wallet_id ? 'selected="selected"' : null ?>><?= $digital_wallet->name ?></option>
			<?php endforeach ?>
        </select>
    </div>

    <div class="form-group">
        <label for="<?= 'digital_wallet_wallet_provider_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-wallet fa-sm text-muted mr-1"></i> <?= l('biolink_digital_wallet.wallet_provider') ?></label>
        <select id="<?= 'digital_wallet_wallet_provider_' . $row->biolink_block_id ?>" name="wallet_provider" class="custom-select" required="required">
            <?php if(digital_wallets_is_google_wallet_ready()): ?>
                <option value="google" <?= ($row->settings->wallet_provider ?? (digital_wallets_is_google_wallet_ready() ? 'google' : 'apple')) == 'google' ? 'selected="selected"' : null ?>><?= l('digital_wallets.google_wallet') ?></option>
            <?php endif ?>
            <?php if(digital_wallets_is_apple_wallet_ready()): ?>
                <option value="apple" <?= ($row->settings->wallet_provider ?? (digital_wallets_is_google_wallet_ready() ? 'google' : 'apple')) == 'apple' ? 'selected="selected"' : null ?>><?= l('digital_wallets.apple_wallet') ?></option>
            <?php endif ?>
            <?php if(digital_wallets_is_google_wallet_ready() && digital_wallets_is_apple_wallet_ready()): ?>
                <option value="both" <?= ($row->settings->wallet_provider ?? (digital_wallets_is_google_wallet_ready() ? 'google' : 'apple')) == 'both' ? 'selected="selected"' : null ?>><?= l('biolink_digital_wallet.wallet_provider_both') ?></option>
            <?php endif ?>
        </select>
    </div>
    ...
    <div class="form-group custom-control custom-switch">
        <input
                id="<?= 'digital_wallet_open_in_new_tab_' . $row->biolink_block_id ?>"
                name="open_in_new_tab" type="checkbox"
                class="custom-control-input"
			<?= ($row->settings->open_in_new_tab ?? false) ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label" for="<?= 'digital_wallet_open_in_new_tab_' . $row->biolink_block_id ?>"><?= l('biolink_link.open_in_new_tab') ?></label>
    </div>
