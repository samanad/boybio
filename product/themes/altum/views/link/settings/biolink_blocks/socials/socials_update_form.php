...
<form name="update_biolink_" method="post" role="form" data-type="<?= $row->type ?>" enctype="multipart/form-data">
    ...
    <div class="form-group">
        <label for="<?= 'socials_color_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-paint-brush fa-sm text-muted mr-1"></i> <?= l('biolink_socials.color') ?></label>
    ...
    <div class="form-group">
        <label for="<?= 'socials_background_color_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-fill fa-sm text-muted mr-1"></i> <?= l('biolink_link.background_color') ?></label>
        <input id="<?= 'socials_background_color_' . $row->biolink_block_id ?>" type="hidden" name="background_color" class="form-control" value="<?= $row->settings->background_color ?>" required="required" />
    ...
    <div class="form-group">
        <label for="<?= 'socials_size_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-expand-alt fa-sm text-muted mr-1"></i> <?= l('biolink_socials.size') ?></label>
        <select id="<?= 'socials_size_' . $row->biolink_block_id ?>" name="size" class="custom-select">
            <option value="s" <?= $row->settings->size == 's' ? 'selected="selected"' : null ?>><?= l('biolink_socials.size.s') ?></option>
            <option value="m" <?= $row->settings->size == 'm' ? 'selected="selected"' : null ?>><?= l('biolink_socials.size.m') ?></option>
            <option value="l" <?= $row->settings->size == 'l' ? 'selected="selected"' : null ?>><?= l('biolink_socials.size.l') ?></option>
            <option value="xl" <?= $row->settings->size == 'xl' ? 'selected="selected"' : null ?>><?= l('biolink_socials.size.xl') ?></option>
        </select>
    </div>
    ...
    <?php foreach($biolink_socials as $key => $value): ?>
        ...
        $social_label_icon = !empty($value['icon_image'])
            ? biolink_social_icon_html($value['icon_image'], 'fa-fw fa-sm mr-1 text-muted')
            : '<i class="' . $value['icon'] . ' fa-fw fa-sm text-muted mr-1"></i>';
        ?>
        <?php if($value['input_group']): ?>
            <div class="form-group">
                <label for="<?= 'socials_' . $key . '_' . $row->biolink_block_id ?>"><?= $social_label_icon ?> <?= l('biolink_socials.' . $key . '.name') ?></label>
                <div class="input-group">
                    <div class="input-group-prepend">
                        <span class="input-group-text"><?= remove_url_protocol_from_url(str_replace('%s', '', $value['format'])) ?></span>
                    </div>
                    <input id="<?= 'socials_' . $key . '_' . $row->biolink_block_id ?>" type="text" class="form-control" name="socials[<?= $key ?>]" placeholder="<?= l('biolink_socials.' . $key . '.placeholder') ?>" value="<?= $row->settings->socials->{$key} ?? '' ?>" maxlength="<?= $value['max_length'] ?>" />
                </div>
            </div>
        <?php else: ?>
            <div class="form-group">
