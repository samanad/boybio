...
<form id="<?= 'update_biolink_block_' . $row->biolink_block_id ?>" name="update_biolink_" method="post" role="form" data-type="<?= $row->type ?>">
    ...
    <input type="hidden" name="block_type" value="bandcamp" />
    <input type="hidden" name="biolink_block_id" value="<?= $row->biolink_block_id ?>" />

    <div class="notification-container"></div>

    <div class="form-group">
        <label for="<?= 'bandcamp_location_url_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-link fa-sm text-muted mr-1"></i> <?= l('biolink_bandcamp.location_url') ?></label>
        <input id="<?= 'bandcamp_location_url_' . $row->biolink_block_id ?>" type="text" class="form-control" name="location_url" value="<?= $row->location_url ?>" maxlength="2048" placeholder="<?= l('biolink_bandcamp.location_url_placeholder') ?>" required="required" />
    </div>

    <div class="form-group">
        <label for="<?= 'bandcamp_type_' . $row->biolink_block_id ?>"><i class="fab fa-fw fa-bandcamp fa-sm text-muted mr-1"></i> <?= l('global.type') ?></label>
        <div class="row btn-group-toggle m-n2" data-toggle="buttons">
            <div class="p-2 col-12 col-lg-6">
                <label class="btn btn-light btn-block font-size-small mb-0 text-truncate <?= $row->settings->type == 'small' ? 'active"' : null?>">
                    <input type="radio" name="type" value="small" class="custom-control-input" <?= $row->settings->type == 'small' ? 'checked="checked"' : null?> required="required" />
                    <i class="fas fa-compress fa-fw fa-sm mr-1"></i> <?= l('biolink_bandcamp.type.small') ?>
                </label>
            </div>

            <div class="p-2 col-12 col-lg-6">
                <label class="btn btn-light btn-block font-size-small mb-0 text-truncate <?= $row->settings->type == 'large' ? 'active"' : null?>">
                    <input type="radio" name="type" value="large" class="custom-control-input" <?= $row->settings->type == 'large' ? 'checked="checked"' : null?> required="required" />
                    <i class="fas fa-expand-arrows-alt fa-fw fa-sm mr-1"></i> <?= l('biolink_bandcamp.type.large') ?>
                </label>
            </div>
        </div>
    </div>
    ...
    <div class="form-group custom-control custom-switch">
        <input
                id="<?= 'bandcamp_display_artwork' . $row->biolink_block_id ?>"
                name="display_artwork" type="checkbox"
                class="custom-control-input"
			<?= $row->settings->display_artwork ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label" for="<?= 'bandcamp_display_artwork' . $row->biolink_block_id ?>"><?= l('biolink_bandcamp.display_artwork') ?></label>
    </div>

    <div class="form-group custom-control custom-switch" data-bandcamp-type="large">
        <input
                id="<?= 'bandcamp_display_track_list' . $row->biolink_block_id ?>"
                name="display_track_list" type="checkbox"
                class="custom-control-input"
			<?= $row->settings->display_track_list ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label" for="<?= 'bandcamp_display_track_list' . $row->biolink_block_id ?>"><?= l('biolink_bandcamp.display_track_list') ?></label>
    </div>

    <div class="form-group">
        <label for="<?= 'link_text_color_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-paint-brush fa-sm text-muted mr-1"></i> <?= l('biolink_link.text_color') ?></label>
    ...
    <div class="form-group">
        <label for="<?= 'link_background_color_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-fill fa-sm text-muted mr-1"></i> <?= l('biolink_link.background_color') ?></label>
...
<script>
    ...
    type_handler('<?= '#update_biolink_block_' . $row->biolink_block_id . ' input[name="type"]' ?>', 'data-bandcamp-type');
    document.querySelector('<?= '#update_biolink_block_' . $row->biolink_block_id . ' input[name="type"]' ?>') && document.querySelectorAll('<?= '#update_biolink_block_' . $row->biolink_block_id . ' input[name="type"]' ?>').forEach(element => element.addEventListener('change', () => { type_handler('<?= '#update_biolink_block_' . $row->biolink_block_id . ' input[name="type"]' ?>', 'data-bandcamp-type'); }));
</script>
<?php \Altum\Event::add_content(ob_get_clean(), 'javascript', 'bandcamp') ?>
