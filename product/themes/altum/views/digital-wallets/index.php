...
<div class="container">
	...
	<?php if($this->user->plan_settings->digital_wallets_limit != -1 && $data->total_digital_wallets > $this->user->plan_settings->digital_wallets_limit): ?>
        <div class="alert alert-danger">
            <i class="fas fa-fw fa-times-circle text-danger mr-2"></i> <?= sprintf(settings()->payment->is_enabled ? l('global.info_message.plan_feature_limit_removal_with_upgrade') : l('global.info_message.plan_feature_limit_removal'), '<strong>' . $data->total_digital_wallets - $this->user->plan_settings->digital_wallets_limit, mb_strtolower(l('digital_wallets.title')) . '</strong>', '<a href="' . url('plan') . '" class="font-weight-bold text-reset">' . l('global.info_message.plan_upgrade') . '</a>') ?>
        </div>
	<?php endif ?>

    <div class="row mb-4">
        <div class="col-12 col-lg d-flex align-items-end mb-3 mb-lg-0 text-truncate">
            <h1 class="h4 m-0 text-truncate"><i class="fas fa-fw fa-xs fa-wifi mr-1"></i> <?= l('digital_wallets.header') ?></h1>

            <div class="ml-2">
                <span data-toggle="tooltip" title="<?= l('digital_wallets.subheader') ?>">
        ...
        <div class="col-12 col-lg-auto d-flex flex-wrap gap-3 d-print-none">
                ...
                <div>
					<?php if($this->user->plan_settings->digital_wallets_limit != -1 && $data->total_digital_wallets >= $this->user->plan_settings->digital_wallets_limit): ?>
                        <button type="button" class="btn btn-primary disabled" <?= get_plan_feature_limit_reached_info() ?>>
                            <i class="fas fa-fw fa-plus-circle fa-sm mr-1"></i> <?= l('digital_wallets.create') ?>
                        </button>
					<?php else: ?>
                        <a href="<?= url('digital-wallet-create') ?>" class="btn btn-primary" data-toggle="tooltip" data-html="true" title="<?= get_plan_feature_limit_info($data->total_digital_wallets, $this->user->plan_settings->digital_wallets_limit, isset($data->filters) ? !$data->filters->has_applied_filters : true) ?>">
                            <i class="fas fa-fw fa-plus-circle fa-sm mr-1"></i> <?= l('digital_wallets.create') ?>
                        </a>
					<?php endif ?>
                </div>
            ...
            <div>
                <div class="dropdown">
                    <button type="button" class="btn btn-light dropdown-toggle-simple <?= !empty($data->digital_wallets) ? null : 'disabled' ?>" data-toggle="dropdown" data-boundary="viewport" data-tooltip title="<?= l('global.export') ?>" data-tooltip-hide-on-click>
                        <i class="fas fa-fw fa-sm fa-download"></i>
                    </button>

                    <div class="dropdown-menu dropdown-menu-right d-print-none">
                        <a href="<?= url('digital-wallets?' . $data->filters->get_get() . '&export=csv')  ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->csv ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->csv ? null : get_plan_feature_disabled_info() ?>>
                            <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                        </a>
                        <a href="<?= url('digital-wallets?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->json ? null : get_plan_feature_disabled_info() ?>>
            ...
            <div>
                <div class="dropdown">
                    <button type="button" class="btn <?= $data->filters->has_applied_filters ? 'btn-dark' : 'btn-light' ?> filters-button dropdown-toggle-simple <?= !empty($data->digital_wallets) || $data->filters->has_applied_filters ? null : 'disabled' ?>" data-toggle="dropdown" data-boundary="viewport" data-tooltip data-html="true" title="<?= l('global.filters.tooltip') ?>" data-tooltip-hide-on-click>
            ...
            <div>
                <button id="bulk_enable" type="button" class="btn btn-light <?= !empty($data->digital_wallets) ? null : 'disabled' ?>" data-toggle="tooltip" title="<?= l('global.bulk_actions') ?>"><i class="fas fa-fw fa-sm fa-list"></i></button>
	...
	<?php if(!empty($data->digital_wallets)): ?>
        ...
        <form id="table" action="<?= SITE_URL . 'digital-wallets/bulk' ?>" method="post" role="form">
                        ...
                        <tr>
                            ...
                            <td class="text-nowrap">
                                <div class="d-flex align-items-center">
                                    <a href="<?= url('digital-wallet-update/' . $row->digital_wallet_id) ?>">
										<?php if($row->settings->logo): ?>
                                            <img src="<?= \Altum\Uploads::get_full_url('digital_wallets') . $row->settings->logo ?>" class="signature-avatar rounded-circle mr-3" alt="<?= $row->name ?>" />
										<?php else: ?>
                                            <div class="signature-avatar rounded-circle mr-3 d-flex align-items-center justify-content-center bg-gray-100">
                            ...
                            <td class="text-nowrap">
                                <div class="d-flex flex-column align-items-start gap-2">
                                    <?php if(digital_wallets_is_google_wallet_ready()): ?>
                                        <a href="<?= url('digital-wallet-add/' . $row->digital_wallet_hash . '?provider=google') ?>" target="_blank" rel="noreferrer">
                                            <img src="<?= ASSETS_FULL_URL . 'images/digital-wallets/add_to_google_wallet.svg' ?>" class="img-fluid" style="width: 8rem;" alt="<?= l('digital_wallets.google_wallet') ?>" />
                                        </a>
                                    <?php endif ?>

                                    <?php if(digital_wallets_is_apple_wallet_ready()): ?>
                                        <a href="<?= url('digital-wallet-add/' . $row->digital_wallet_hash . '?provider=apple') ?>" target="_blank" rel="noreferrer">
                                            <img src="<?= ASSETS_FULL_URL . 'images/digital-wallets/add_to_apple_wallet.svg' ?>" class="img-fluid" style="width: 8rem;" alt="<?= l('digital_wallets.apple_wallet') ?>" />
                                        </a>
                                    <?php endif ?>
                                </div>
                            </td>
                            ...
                            <td>
                                <div class="d-flex justify-content-end">
									<?= include_view(THEME_PATH . 'views/digital-wallets/digital_wallet_dropdown_button.php', ['id' => $row->digital_wallet_id, 'hash' => $row->digital_wallet_hash, 'resource_name' => $row->name]) ?>
                                </div>
                            </td>
                        </tr>
					<?php endforeach ?>
