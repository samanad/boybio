...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 mb-3 mb-md-0 text-truncate"><i class="fas fa-fw fa-xs fa-icons text-primary-900 mr-2"></i> <?= l('admin_images.header') ?></h1>
        ...
        <div>
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right d-print-none">
                    <a href="<?= url('admin/images?' . $data->filters->get_get() . '&export=csv') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->csv ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->csv ? null : get_plan_feature_disabled_info() ?>>
                        <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                    </a>
                    <a href="<?= url('admin/images?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->json ? null : get_plan_feature_disabled_info() ?>>
                    ...
                    <form action="" method="get" role="form">
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="size" id="filters_size" class="custom-select custom-select-sm">
                                <option value=""><?= l('global.all') ?></option>
                                <?php foreach(['256x256', '512x512', '1024x1024', '1792x1024', '1024x1792'] as $key): ?>
                        ...
                        <div class="form-group px-4">
                            <label for="filters_api" class="small"><?= l('images.api') ?></label>
                            <select name="api" id="filters_api" class="custom-select custom-select-sm">
                                <option value=""><?= l('global.all') ?></option>
                                <?php foreach(require \Altum\Plugin::get('aix')->path . 'includes/ai_image_models.php' as $key => $value): ?>
                                    <option value="<?= $key ?>" <?= isset($data->filters->filters['api']) && $data->filters->filters['api'] == str_replace('-', '_', $key) ? 'selected="selected"' : null ?>><?= $value['name'] ?></option>
                                <?php endforeach ?>
                            </select>
                        </div>
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="order_by" id="filters_order_by" class="custom-select custom-select-sm">
                                <option value="image_id" <?= $data->filters->order_by == 'image_id' ? 'selected="selected"' : null ?>><?= l('global.id') ?></option>
        ...
        <table class="table table-custom">
            ...
            <?php foreach($data->images as $row): ?>
                ...
                <tr>
                    ...
                    <td class="text-nowrap">
                        <div class="d-flex align-items-center">
                        <span class="mr-2" data-toggle="tooltip" data-html="true" title="<?= l('images.api') . '<br />' . $row->api ?>">
                    ...
                    <td>
                        <div class="d-flex justify-content-end">
                            <?= include_view(THEME_PATH . 'views/admin/images/admin_image_dropdown_button.php', ['id' => $row->image_id, 'resource_name' => $row->name, 'image' => $row->image, 'image_url' => \Altum\Uploads::get_full_url('images') . $row->image]) ?>
                        </div>
                    </td>
                </tr>
            <?php endforeach ?>
