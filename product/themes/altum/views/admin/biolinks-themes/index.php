...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 mb-3 mb-md-0 text-truncate"><i class="fas fa-fw fa-xs fa-palette text-primary-900 mr-2"></i> <?= l('admin_biolinks_themes.header') ?></h1>

    <div class="d-flex position-relative d-print-none">
        <div>
            <a href="<?= url('admin/biolink-theme-create') ?>" class="btn btn-primary text-nowrap"><i class="fas fa-fw fa-plus-circle fa-sm mr-1"></i> <?= l('admin_biolink_theme_create.menu') ?></a>
        </div>
        ...
        <div class="ml-3">
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right d-print-none">
                    <a href="<?= url('admin/biolinks-themes?' . $data->filters->get_get() . '&export=csv') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->csv ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->csv ? null : get_plan_feature_disabled_info() ?>>
                        <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                    </a>
                    <a href="<?= url('admin/biolinks-themes?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->json ? null : get_plan_feature_disabled_info() ?>>
                    ...
                    <form action="" method="get" role="form">
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="order_by" id="filters_order_by" class="custom-select custom-select-sm">
                                <option value="biolink_theme_id" <?= $data->filters->order_by == 'biolink_theme_id' ? 'selected="selected"' : null ?>><?= l('global.id') ?></option>
    ...
    <div class="table-responsive table-custom-container">
        <table class="table table-custom">
            ...
            <tr>
                ...
                <th><?= l('global.name') ?></th>
                <th><?= l('admin_biolinks_themes.total_usage') ?></th>
            ...
            <?php foreach($data->biolinks_themes as $row): ?>
                ...
                <tr>
                    ...
                    <td class="text-nowrap">
                        <a href="<?= url('admin/links?biolink_theme_id=' . $row->biolink_theme_id) ?>" class="badge badge-light"><i class="fas fa-fw fa-sm fa-hashtag mr-1"></i> <?= nr($row->total_usage) ?></a>
                    </td>
