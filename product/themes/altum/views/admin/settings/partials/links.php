...
<div>
    ...
    <div class="collapse" id="biolinks_container">
        ...
        <div class="form-group custom-control custom-switch">
            <input id="branding_edit_link_is_enabled" name="branding_edit_link_is_enabled" type="checkbox" class="custom-control-input" <?= (isset(settings()->links->branding_edit_link_is_enabled) && settings()->links->branding_edit_link_is_enabled) ? 'checked="checked"' : null?>>
            <label class="custom-control-label" for="branding_edit_link_is_enabled"><i class="fas fa-fw fa-sm fa-edit text-muted mr-1"></i> <?= l('admin_settings.links.branding_edit_link_is_enabled') ?></label>
            <small class="form-text text-muted"><?= l('admin_settings.links.branding_edit_link_is_enabled_help') ?></small>
        </div>
                ...
                <?php foreach($biolink_blocks as $key => $value): ?>
                    <div class="col-12 col-lg-6">
                        <div class="custom-control custom-checkbox my-2">
                            <input id="<?= 'available_biolink_blocks_' . $key ?>" name="available_biolink_blocks[]" value="<?= $key ?>" type="checkbox" class="custom-control-input" <?= (isset(settings()->links->available_biolink_blocks->{$key}) && settings()->links->available_biolink_blocks->{$key}) ? 'checked="checked"' : null ?>>
    ...
    <div class="collapse" id="file_size_limits_container">
        <?php foreach(['avatar', 'background', 'favicon', 'seo_image', 'thumbnail_image', 'image', 'audio', 'video', 'file', 'product_file', 'static', 'pwa_icon'] as $key): ?>
            <div class="form-group">
                ...
                <div class="input-group">
                    <input id="<?= $key . '_size_limit' ?>" type="number" min="0" step="any" name="<?= $key . '_size_limit' ?>" class="form-control" value="<?= settings()->links->{$key . '_size_limit'} ?>" />
                    <div class="input-group-append">
...
<button type="submit" name="submit" id="admin_settings_links_submit" class="btn btn-lg btn-block btn-primary mt-4"><?= l('global.update') ?></button>
...
<script>
    ...
    (function() {
        const form = document.querySelector('form[action*="admin/settings/links"]');
        const submitButton = document.getElementById('admin_settings_links_submit');
        ...
        /* Prevent unwanted scrolling when clicking collapsible sections */
        document.querySelectorAll('[data-toggle="collapse"]').forEach(toggle => {
            /* Make sure this is actually a collapse toggle button, not the submit button */
            if(toggle === submitButton || toggle.id === 'admin_settings_links_submit' || toggle.type === 'submit') {
                return; /* Skip the submit button */
            }
            
            toggle.addEventListener('click', function(e) {
                /* Make sure we're not clicking the submit button */
                if(e.target === submitButton || e.target.id === 'admin_settings_links_submit' || e.target.type === 'submit' || e.target.name === 'submit') {
                    return; /* Don't handle collapse toggle behavior for submit button */
                }
                
                /* Don't scroll when opening sections - let the user stay where they are */
                /* The submit button will be visible when they scroll down naturally */
            });
        });
        ...
        form.addEventListener('submit', function(event) {
            ...
            if(!form.checkValidity()) {
                ...
                invalidFields.forEach(field => {
                    ...
                    const collapseContainer = field.closest('.collapse');
                    
                    if(collapseContainer && !$(collapseContainer).hasClass('show')) {
                        /* Open the collapsed section */
                        $(collapseContainer).collapse('show');
                        
                        /* Wait for collapse animation, then scroll to field */
                        $(collapseContainer).one('shown.bs.collapse', () => {
                            field.scrollIntoView({ behavior: 'smooth', block: 'center' });
                            setTimeout(() => field.focus(), 100);
                        });
                    } else if(collapseContainer && $(collapseContainer).hasClass('show')) {
                        /* Section already open, just scroll to field */
                        field.scrollIntoView({ behavior: 'smooth', block: 'center' });
                        setTimeout(() => field.focus(), 100);
                    } else {
                        /* No collapse container, just scroll to field */
                        field.scrollIntoView({ behavior: 'smooth', block: 'center' });
                        setTimeout(() => field.focus(), 100);
                    }
                });
            ...
            /* Prevent any collapse toggles from interfering */
            event.stopPropagation();
            
            /* Show loading state */
            submitButton.disabled = true;
            submitButton.innerHTML = '<i class="fas fa-fw fa-spinner fa-spin mr-1"></i> <?= addslashes(l('global.loading')) ?>';
            
            /* Scroll button into view if it's not visible */
            ...
            /* The form will redirect on success or show errors */
        });
        
        /* Also prevent collapse toggle on submit button click */
        submitButton.addEventListener('click', function(e) {
            e.stopPropagation();
            /* Don't prevent default - let form submit normally */
        });
        
        /* Re-enable button if form validation fails (page doesn't reload) */
