        ...
        <?php foreach(require APP_PATH . 'includes/admin_socials.php' AS $key => $value): ?>
            <div class="form-group">
                ...
                <div class="input-group">
                    ...
                    <?php endif ?>
                    <input id="<?= $key ?>" type="text" name="<?= $key ?>" class="form-control" value="<?= isset(settings()->socials->{$key}) ? settings()->socials->{$key} : '' ?>" placeholder="<?= $value['placeholder'] ?>" />
                </div>
            </div>
        <?php endforeach ?>
    </div>
            ...
            <?php foreach($social_share_buttons as $key => $value): ?>
                <div class="col-12 col-lg-6">
                    <div class="custom-control custom-checkbox my-2">
                        <input id="<?= 'share_button_' . $key ?>" name="<?= 'share_button_' . $key ?>" type="checkbox" class="custom-control-input" <?= (isset(settings()->socials->share_buttons) && isset(settings()->socials->share_buttons->{$key}) && settings()->socials->share_buttons->{$key}) ? 'checked="checked"' : null ?>>
