...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 mb-3 mb-md-0 text-truncate"><i class="fas fa-fw fa-xs fa-droplet text-primary-900 mr-2"></i> <?= l('admin_splash_pages.header') ?></h1>

    <div class="d-flex position-relative d-print-none">
        <div class="ml-3">
            <a href="<?= url('admin/statistics/splash_pages') ?>" class="btn btn-gray-300" data-tooltip title="<?= l('global.statistics') ?>">
        ...
        <div class="ml-3">
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right d-print-none">
                    <a href="<?= url('admin/splash-pages?' . $data->filters->get_get() . '&export=csv') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->csv ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->csv ? null : get_plan_feature_disabled_info() ?>>
                        <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                    </a>
                    <a href="<?= url('admin/splash-pages?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->json ? null : get_plan_feature_disabled_info() ?>>
...
<form id="table" action="<?= SITE_URL . 'admin/splash-pages/bulk' ?>" method="post" role="form">
        ...
        <table class="table table-custom">
            ...
            <?php foreach($data->splash_pages as $row): ?>
                ...
                <tr>
                    ...
                    <td class="text-nowrap">
                        <span class="badge badge-light" data-toggle="tooltip" title="<?= l('splash_pages.link_unlock_seconds') ?>">
                    ...
                    <td>
                        <div class="d-flex justify-content-end">
                            <?= include_view(THEME_PATH . 'views/admin/splash-pages/admin_splash_page_dropdown_button.php', ['id' => $row->splash_page_id, 'resource_name' => $row->name]) ?>
                        </div>
                    </td>
                </tr>
            <?php endforeach ?>
...
<?php \Altum\Event::add_content(include_view(THEME_PATH . 'views/partials/universal_delete_modal_url.php', [
    ...
    'path' => 'admin/splash-pages/delete/'
]), 'modals'); ?>
