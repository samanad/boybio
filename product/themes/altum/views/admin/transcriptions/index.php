...
<?php if(!settings()->aix->transcriptions_is_enabled): ?>
    <div class="alert alert-info">
        ...
        <?= sprintf(l('global.info_message.admin_feature_disabled'), url('admin/settings/aix')) ?>
    </div>
<?php endif ?>

<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 mb-3 mb-md-0 text-truncate"><i class="fas fa-fw fa-xs fa-microphone-alt text-primary-900 mr-2"></i> <?= l('admin_transcriptions.header') ?></h1>
        ...
        <div>
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right d-print-none">
                    <a href="<?= url('admin/transcriptions?' . $data->filters->get_get() . '&export=csv') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->csv ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->csv ? null : get_plan_feature_disabled_info() ?>>
                        <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                    </a>
                    <a href="<?= url('admin/transcriptions?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->json ? null : get_plan_feature_disabled_info() ?>>
            ...
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right filters-dropdown">
                    ...
                    <form action="" method="get" role="form">
                        ...
                        <div class="form-group px-4">
                            <label for="filters_language" class="small"><?= l('global.language') ?></label>
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="order_by" id="filters_order_by" class="custom-select custom-select-sm">
                                <option value="transcription_id" <?= $data->filters->order_by == 'transcription_id' ? 'selected="selected"' : null ?>><?= l('global.id') ?></option>
...
<form id="table" action="<?= SITE_URL . 'admin/transcriptions/bulk' ?>" method="post" role="form">
            ...
            <?php foreach($data->transcriptions as $row): ?>
                ...
                <tr>
                    <td data-bulk-table class="d-none">
                        <div class="custom-control custom-checkbox">
                            <input id="selected_transcription_id_<?= $row->transcription_id ?>" type="checkbox" class="custom-control-input" name="selected[]" value="<?= $row->transcription_id ?>" />
                            <label class="custom-control-label" for="selected_transcription_id_<?= $row->transcription_id ?>"></label>
                        </div>
                    </td>
                    ...
                    <td class="text-nowrap">
                        ...
                        <span class="mr-2" data-toggle="tooltip" data-html="true" title="<?= l('transcriptions.words') . '<br />' . nr($row->words) ?>">
                            <i class="fas fa-fw fa-feather text-muted"></i>
                        </span>

                            <span class="mr-2" data-toggle="tooltip" data-html="true" title="<?= l('transcriptions.api_response_time') . '<br />' . display_response_time($row->api_response_time) ?>">
                    ...
                    <td>
                        <div class="d-flex justify-content-end">
                            <?= include_view(THEME_PATH . 'views/admin/transcriptions/admin_transcription_dropdown_button.php', ['id' => $row->transcription_id, 'resource_name' => $row->name]) ?>
                        </div>
                    </td>
                </tr>
            <?php endforeach ?>
