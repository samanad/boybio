...
moment.tz.setDefault(<?= json_encode($this->user->timezone) ?>);
    ...
    let check_plan_id = () => {
        ...
        let plan_settings = document.querySelector('#plan_settings');
        ...
        if(selected_plan_id == 'custom') {
            plan_settings.style.display = 'block';
            /* Enable all inputs in plan_settings when visible */
            plan_settings.querySelectorAll('input, select, textarea').forEach(element => {
                element.disabled = false;
                element.removeAttribute('data-was-disabled');
            });
        } else {
            plan_settings.style.display = 'none';
            /* Disable all inputs in plan_settings when hidden to prevent validation errors */
            plan_settings.querySelectorAll('input, select, textarea').forEach(element => {
                if(!element.disabled) {
                    element.setAttribute('data-was-disabled', 'false');
                }
    ...
    /* Handle form submission - ensure hidden fields don't block validation */
    document.querySelector('form[role="form"]').addEventListener('submit', function(e) {
        let plan_settings = document.querySelector('#plan_settings');
        if(plan_settings.style.display === 'none') {
            /* Temporarily remove validation constraints from hidden fields */
            plan_settings.querySelectorAll('input[type="number"], input[type="text"], input[type="email"], select, textarea').forEach(element => {
                if(element.hasAttribute('min')) element.removeAttribute('min');
                if(element.hasAttribute('max')) element.removeAttribute('max');
                if(element.hasAttribute('required')) element.removeAttribute('required');
            });
        }
    });
