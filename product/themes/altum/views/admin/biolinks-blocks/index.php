...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 mb-3 mb-md-0 text-truncate"><i class="fas fa-fw fa-xs fa-table-cells-large text-primary-900 mr-2"></i> <?= l('admin_biolinks_blocks.header') ?></h1>

    <div class="d-flex position-relative d-print-none">
        <div class="ml-3">
            <a href="<?= url('admin/statistics/biolinks_blocks') ?>" class="btn btn-gray-300" data-tooltip title="<?= l('global.statistics') ?>">
        ...
        <div class="ml-3">
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right d-print-none">
                    <a href="<?= url('admin/biolinks-blocks?' . $data->filters->get_get() . '&export=csv') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->csv ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->csv ? null : get_plan_feature_disabled_info() ?>>
                        <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                    </a>
                    <a href="<?= url('admin/biolinks-blocks?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->json ? null : get_plan_feature_disabled_info() ?>>
                    ...
                    <form action="" method="get" role="form">
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="type" id="filters_type" class="custom-select custom-select-sm">
                                <option value=""><?= l('global.all') ?></option>
                                <?php foreach($data->biolink_blocks as $key => $value): ?>
                                    <option value="<?= $key ?>" <?= isset($data->filters->filters['type']) && $data->filters->filters['type'] == $key ? 'selected="selected"' : null ?>><?= l('link.biolink.blocks.' . mb_strtolower($key)) ?></option>
                                <?php endforeach ?>
                            </select>
                        </div>
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="order_by" id="filters_order_by" class="custom-select custom-select-sm">
                                <option value="biolink_block_id" <?= $data->filters->order_by == 'biolink_block_id' ? 'selected="selected"' : null ?>><?= l('global.id') ?></option>
...
<form id="table" action="<?= SITE_URL . 'admin/biolinks-blocks/bulk' ?>" method="post" role="form">
    ...
    <table class="table table-custom">
        ...
        <tr>
            ...
            <th></th>
            <th><?= l('global.type') ?></th>
            <th><?= l('admin_links.table.clicks') ?></th>
        ...
        <tbody>
        <?php foreach($data->biolinks_blocks as $row): ?>
            <?php //ALTUMCODE:DEMO if(DEMO) {$row->user_email = 'hidden@demo.com'; $row->user_name = 'hidden on demo';} ?>
            <tr>
                <td data-bulk-table class="d-none">
                    <div class="custom-control custom-checkbox">
                        <input id="selected_biolink_block_id_<?= $row->biolink_block_id ?>" type="checkbox" class="custom-control-input" name="selected[]" value="<?= $row->biolink_block_id ?>" />
                        <label class="custom-control-label" for="selected_biolink_block_id_<?= $row->biolink_block_id ?>"></label>
                    </div>
                </td>
                ...
                <td class="text-nowrap">
                    <span><?= $data->biolink_blocks[$row->type]['display_dynamic_name'] ? ($row->settings->{$data->biolink_blocks[$row->type]['display_dynamic_name']} ? string_truncate(strip_tags($row->settings->{$data->biolink_blocks[$row->type]['display_dynamic_name']}), 32) : l('link.biolink.blocks.' . $row->type)) : l('link.biolink.blocks.' . $row->type) ?></span>

                    <a href="<?= url('link-redirect/' . $row->link_id) ?>" target="_blank" rel="noreferrer">
                ...
                <td class="text-nowrap text-muted">
                    <a href="<?= url('admin/links?link_id=' . $row->link_id) ?>" class="mr-2" data-toggle="tooltip" title="<?= l('links.title') ?>">
                ...
                <td>
                    <div class="d-flex justify-content-end">
                        <?= include_view(THEME_PATH . 'views/admin/biolinks-blocks/admin_biolink_block_dropdown_button.php', ['id' => $row->biolink_block_id]) ?>
                    </div>
                </td>
            </tr>
        <?php endforeach ?>
