...
<html lang="<?= \Altum\Language::$default_code ?>" class="link-html<?= $biolink_html_tool_classes ?>" dir="<?= l('direction') ?>">
    <head>
        ...
        <?php if(count($biolink_tools_enabled)): ?>
            <link href="<?= ASSETS_FULL_URL . 'css/biolink-tools.css?v=' . PRODUCT_CODE . '-nopw' ?>" rel="stylesheet" media="screen,print">
        <?php endif ?>
    ...
    <?php if(count($biolink_tools_enabled ?? [])): ?>
        <script src="<?= ASSETS_FULL_URL ?>js/biolink-tools.js?v=<?= PRODUCT_CODE ?>-nopw"></script>
    <?php endif ?>
